...
docu = contextpatt.load_text("data/sents_train0.json")  # raw text document
patt = contextpatt.load_pattern("data/patternlist.xlsx") # patterns
if REUSE:
    patt = contextpatt.filter_exist("data/train0_context/") # remains patterns
matches = []
...
def run (pattern):
    ...
    with open("data/train0_context/" + str(id) + ":" + "_".join(p.split(" ")) + ".json", "w") as f:
        json.dump(dict, f)
    return
